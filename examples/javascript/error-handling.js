/**
 * Dog Breeds API - Error Handling Example
 *
 * This example demonstrates proper error handling when using the Dog Breeds API.
 * API Documentation: https://docs.apiverve.com/ref/dogbreeds
 */

const API_KEY = process.env.APIVERVE_API_KEY || 'YOUR_API_KEY_HERE';
const API_URL = 'https://api.apiverve.com/v1/dogbreeds';
 ...
 */
async function callDogBreedsAPIWithErrorHandling(queryParams = {}) {
  try {
...
async function callWithRetry(maxRetries = 3, initialDelay = 1000) {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      const result = await callDogBreedsAPIWithErrorHandling({
